...
fn benchmark(c: &mut Criterion) {
    c.bench_function("push pop sling 4", |b| {
        b.iter(|| push_pop_sling(RingBuffer::<u8, BUF_LEN>::new(), 4))
    });

    c.bench_function("push pop sling 8", |b| {
        b.iter(|| push_pop_sling(RingBuffer::<u8, BUF_LEN>::new(), 8))
    });

    c.bench_function("push pop sling 16", |b| {
        b.iter(|| push_pop_sling(RingBuffer::<u8, BUF_LEN>::new(), 16))
    });

    c.bench_function("push pop lockfree 4", |b| b.iter(|| push_pop_lockfree(4)));

    c.bench_function("push pop lockfree 8", |b| b.iter(|| push_pop_lockfree(8)));

    c.bench_function("push pop lockfree 16", |b| b.iter(|| push_pop_lockfree(16)));
}
